...
from app.models.schemas import DashboardStatsSchema, DomainStatSchema
from app.models.db_models import Document, Chunk, Session as SessionModel, Message
...
async def dashboard_stats(db: Session = Depends(get_db)):
    ...
    if pending_count > 0:
        ...
        indexing_status = f"{indexed_count} indexés"
    else:
...
def domain_stats(db: Session = Depends(get_db)):
    ...
    for domain, doc_count in domain_docs:
        ...
        result.append(DomainStatSchema(
            domain=domain,
